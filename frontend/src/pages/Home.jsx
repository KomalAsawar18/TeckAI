  ...
  return (
    <div className="card featured-horizontal-card flex align-center gap-4 p-4 relative">
      ...
      <div className="featured-info flex flex-col flex-grow">
        <div className="featured-badge text-xs font-semibold mb-1 w-fit">Featured</div>
        <span className="text-xs text-muted font-semibold uppercase tracking-wider">{brand}</span>
        <h3 className="text-sm font-bold mb-1 truncate-2-lines">
...
const Home = () => {
  ...
  const getCategoryIcon = (slug) => {
    switch (slug.toLowerCase()) {
      case 'laptops': return <Laptop size={28} className="text-accent-highlight" strokeWidth={1.5} />;
      case 'headphones': return <Headphones size={28} className="text-accent-highlight" strokeWidth={1.5} />;
      case 'keyboards': return <Keyboard size={28} className="text-accent-highlight" strokeWidth={1.5} />;
      case 'monitors': return <Monitor size={28} className="text-accent-highlight" strokeWidth={1.5} />;
      default: return <Cpu size={28} className="text-accent-highlight" strokeWidth={1.5} />;
    }
  };
      ...
      <section className="container mt-6 mb-12">
        <div className="premium-hero-banner flex">
          <div className="hero-content flex flex-col justify-center">
            ...
            <form onSubmit={handleSearchSubmit} className="hero-search-form flex align-center bg-surface w-full max-w-lg">
              <div className="search-input-wrapper flex flex-grow align-center px-4 gap-2">
              ...
              <button
                type="button"
                className="btn btn-primary hero-ai-btn flex align-center gap-2 h-full"
                onClick={() => {
      ...
      {categories.length > 0 && (
        <section className="container mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categories.map((cat) => (
              <Link key={cat.slug} to={`/products?category=${cat.slug}`} className="card category-clean-card flex align-center p-6 justify-between group">
